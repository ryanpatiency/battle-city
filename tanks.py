...
class Bonus():
    ...
    # bonus types
    (BONUS_GRENADE, BONUS_HELMET, BONUS_SHOVEL,
     BONUS_STAR, BONUS_TANK, BONUS_TIMER) = list(range(6))
    ...
    def __init__(self, level):
        ...
        self.rect = pygame.Rect(random.randint(
            0, 416-32), random.randint(0, 416-32), 32, 32)
    ...
    def explode(self):
        ...
        if self.state != self.STATE_REMOVED:
            ...
            self.explosion = Explosion(
                [self.rect.left-13, self.rect.top-13], None, self.explosion_images)
...
class Level():
    ...
    # tile constants
    (TILE_EMPTY, TILE_BRICK, TILE_STEEL, TILE_WATER,
     TILE_GRASS, TILE_FROZE) = list(range(6))
    ...
    def loadLevel(self, level_nr=1):
        ...
        for row in data:
            for ch in row:
                if ch == "#":
                    self.mapr.append(
                        myRect(x, y, self.TILE_SIZE, self.TILE_SIZE, self.TILE_BRICK))
                elif ch == "@":
                    self.mapr.append(
                        myRect(x, y, self.TILE_SIZE, self.TILE_SIZE, self.TILE_STEEL))
                elif ch == "~":
                    self.mapr.append(
                        myRect(x, y, self.TILE_SIZE, self.TILE_SIZE, self.TILE_WATER))
                elif ch == "%":
                    self.mapr.append(
                        myRect(x, y, self.TILE_SIZE, self.TILE_SIZE, self.TILE_GRASS))
                elif ch == "-":
                    self.mapr.append(
                        myRect(x, y, self.TILE_SIZE, self.TILE_SIZE, self.TILE_FROZE))
                x += self.TILE_SIZE
            x = 0
            y += self.TILE_SIZE
        return True
    ...
    def draw(self, tiles=None):
        ...
        if tiles == None:
            tiles = [TILE_BRICK, TILE_STEEL,
                     TILE_WATER, TILE_GRASS, TILE_FROZE]
    ...
    def buildFortress(self, tile):
        ...
        for pos in positions:
            self.mapr.append(
                myRect(pos[0], pos[1], self.TILE_SIZE, self.TILE_SIZE, tile))
...
class Tank():
    ...
    def __init__(self, level, side, position=None, direction=None, filename=None):
        ...
        self.controls = [pygame.K_SPACE, pygame.K_UP,
        ...
        # spawning animation
        self.timer_uuid_spawn = gtimer.add(
            100, lambda: self.toggleSpawnImage())

        # duration of spawning
        self.timer_uuid_spawn_end = gtimer.add(
    ...
    def bulletImpact(self, friendly_fire=False, damage=100, tank=None):
        ...
        if self.side == self.SIDE_ENEMY:
            ...
            if not self.paralised:
                ...
                self.timer_uuid_paralise = gtimer.add(
                    10000, lambda: self.setParalised(False), 1)
            return True
...
class Enemy(Tank):
    ...
    def __init__(self, level, type, position=None, direction=None, filename=None):
        ...
        Tank.__init__(self, level, type, position=None,
                      direction=None, filename=None)
    ...
    def getFreeSpawningPosition(self):
        ...
        available_positions = [
            [(self.level.TILE_SIZE * 2 - self.rect.width) / 2,
    ...
    def generatePath(self, direction=None, fix_direction=False):
        ...
        all_directions = [self.DIR_UP, self.DIR_RIGHT,
...
class Game():
    ...
    def __init__(self):
        ...
        self.player_image = pygame.transform.rotate(
            sprites.subsurface(0, 0, 13*2, 13*2), 270)
    ...
    def triggerBonus(self, bonus, player):
        ...
        if bonus.bonus == bonus.BONUS_GRENADE:
            ...
            gtimer.add(10000, lambda: self.level.buildFortress(
                self.level.TILE_BRICK), 1)
    ...
    def shieldPlayer(self, player, shield=True, duration=None):
        ...
        if shield:
            player.timer_uuid_shield = gtimer.add(
                100, lambda: player.toggleShieldImage())
        else:
    ...
    def showScores(self):
        ...
        screen.blit(self.font.render(
            "STAGE"+str(self.stage).rjust(3), False, white), [170, 65])

        screen.blit(self.font.render("I-PLAYER", False, purple), [25, 95])

        # player 1 global score
        screen.blit(self.font.render(
            str(players[0].score).rjust(8), False, pink), [25, 125])

        if self.nr_of_players == 2:
            screen.blit(self.font.render(
                "II-PLAYER", False, purple), [310, 95])

            # player 2 global score
            screen.blit(self.font.render(
                str(players[1].score).rjust(8), False, pink), [325, 125])
        ...
        for i in range(4):
            ...
            for n in range(tanks+1):
                ...
                # erase previous text
                screen.blit(self.font.render(str(n-1).rjust(2),
                                             False, black), [170, 168+(i*45)])
                # print new number of enemies
                screen.blit(self.font.render(str(n).rjust(2),
                                             False, white), [170, 168+(i*45)])
                # erase previous text
                screen.blit(self.font.render(
                    str((n-1) * (i+1) * 100).rjust(4)+" PTS", False, black), [25, 168+(i*45)])
                # print new total points per enemy
                screen.blit(self.font.render(
                    str(n * (i+1) * 100).rjust(4)+" PTS", False, white), [25, 168+(i*45)])
                pygame.display.flip()
            ...
            if self.nr_of_players == 2:
                ...
                for n in range(tanks+1):
                    ...
                    screen.blit(self.font.render(str(n-1).rjust(2),
                                                 False, black), [277, 168+(i*45)])
                    screen.blit(self.font.render(str(n).rjust(2),
                                                 False, white), [277, 168+(i*45)])

                    screen.blit(self.font.render(
                        str((n-1) * (i+1) * 100).rjust(4)+" PTS", False, black), [325, 168+(i*45)])
                    screen.blit(self.font.render(
                        str(n * (i+1) * 100).rjust(4)+" PTS", False, white), [325, 168+(i*45)])
        ...
        # total tanks
        tanks = sum([i for i in list(players[0].trophies.values())]
                    ) - players[0].trophies["bonus"]
        screen.blit(self.font.render(
            str(tanks).rjust(2), False, white), [170, 335])
        if self.nr_of_players == 2:
            tanks = sum([i for i in list(players[1].trophies.values())]
                        ) - players[1].trophies["bonus"]
            screen.blit(self.font.render(
                str(tanks).rjust(2), False, white), [277, 335])
    ...
    def draw(self):
        ...
        self.level.draw([self.level.TILE_EMPTY, self.level.TILE_BRICK,
                         self.level.TILE_STEEL, self.level.TILE_FROZE, self.level.TILE_WATER])
    ...
    def drawSidebar(self):
        ...
        if pygame.font.get_init():
            ...
            for n in range(len(players)):
                if n == 0:
                    screen.blit(self.font.render(
                        str(n+1)+"P", False, text_color), [x+16, y+200])
                    screen.blit(self.font.render(
                        str(players[n].lives), False, text_color), [x+31, y+215])
                    screen.blit(self.player_life_image, [x+17, y+215])
                else:
                    screen.blit(self.font.render(
                        str(n+1)+"P", False, text_color), [x+16, y+240])
                    screen.blit(self.font.render(
                        str(players[n].lives), False, text_color), [x+31, y+255])
                    screen.blit(self.player_life_image, [x+17, y+255])

            screen.blit(self.flag_image, [x+17, y+280])
            screen.blit(self.font.render(str(self.stage),
    ...
    def drawIntroScreen(self, put_on_surface=True):
        ...
        if pygame.font.get_init():
            ...
            screen.blit(self.font.render("HI- "+str(hiscore),
                                         True, pygame.Color('white')), [170, 35])

            screen.blit(self.font.render("1 PLAYER", True,
                                         pygame.Color('white')), [165, 250])
            screen.blit(self.font.render("2 PLAYERS", True,
                                         pygame.Color('white')), [165, 275])

            screen.blit(self.font.render("(c) 1980 1985 NAMCO LTD.",
                                         True, pygame.Color('white')), [50, 350])
            screen.blit(self.font.render("ALL RIGHTS RESERVED",
                                         True, pygame.Color('white')), [85, 380])
    ...
    def nextLevel(self):
        ...
        self.level.enemies_left = [0]*enemies_l[0] + [1] * \
